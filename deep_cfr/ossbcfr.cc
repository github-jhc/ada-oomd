...
void actor(const Game& game, const DeepCFRConfig& config, int num,
  ...
  DeepOSSBCFRSolver solver(game, value_0_eval, value_1_eval, policy_0_eval,
    ...
    if (config.use_tabular) {
      ...
      for (auto& infoset : infosets) {
        ...
        std::vector<double> tabular_value =
            model->GetCFRTabularValue({inf_input}, true);
    ...
    if (trajectory) {
      ...
      if (value_or_policy) {
        ...
        for (ReplayNode& n : trajectory->states) {
          ...
          PolicyReplayNode policy_node{n.information, n.current_player,
  ...
  if (true) {
    ...
    if (value_or_policy) {
      ...
      for (auto& node : last_nodes) {
        last_inputs.emplace_back(
            CFRNetModel::InferenceInputs{node.legal_actions, node.information});
      }
      ...
      Alpha = alpha;
      if (step % 100000 == 0) {
        logger.Print("alpha: %.3f, value: %.3f, cfr_value: [%.3f, %.3f]", alpha,
                     value, cfr_value[0], cfr_value[1]);
        std::cout << "alpha" << alpha << " " << value << cfr_value << std::endl;
      }
    }
      ...
      if (value_or_policy) {
        ...
        for (int i = 0; i != data.size(); ++i) {
          ...
          target_inference_inputs.emplace_back(CFRNetModel::InferenceInputs{
              node.next_legal_actions, node.next_information});
        }
        target_outputs =
        ...
        for (int i = 0; i != data.size(); ++i) {
          ...
          train_inputs.emplace_back(CFRNetModel::TrainInputs{
              legal_actions, node.information, target_value, 1});
        }
      } else {
        ...
        for (int i = 0; i != data.size(); ++i) {
          ...
          train_inputs.emplace_back(CFRNetModel::TrainInputs{
              node.legal_actions, node.information, value, node.weight});
        }
      }
      {
        auto start = std::chrono::system_clock::now();
