...
VPNetEvaluator::VPNetEvaluator(DeviceManager *device_manager,
  ...
  if (cache_size != 0) {
    ...
    for (int i = 0; i < cache_shards; ++i) {
      cache_.push_back(
          std::unique_ptr<LRUCache<uint64_t, CFRNetModel::InferenceOutputs>>(
              new LRUCache<uint64_t, CFRNetModel::InferenceOutputs>(
...
CFRNetModel::InferenceOutputs VPNetEvaluator::Inference(const State &state) {
  CFRNetModel::InferenceInputs inputs = {state.LegalActions(),
...
CFRNetModel::InferenceOutputs VPNetEvaluator::Inference(
  ...
  int cache_shard;
  if (!cache_.empty()) {
    key = absl::Hash<CFRNetModel::InferenceInputs>{}(inputs);
...
void VPNetEvaluator::SetCFRTabular(const State &state,
                                   ...
                                   bool set_true) {
  SetCFRTabular(CFRNetModel::TrainInputs{state.LegalActions(),
                                         state.InformationStateTensor(), value},
                set_true);
}
...
void VPNetEvaluator::SetCFRTabular(const CFRNetModel::TrainInputs &input,
  ...
  for (auto &device : device_manager_.GetAll()) {
    if (value_or_policy) {
      device->SetCFRTabularValue(input, set_true);
    } else {
      device->SetCFRTabularPolicy(input, set_true);
    }
  }
}
...
void VPNetEvaluator::AccumulateCFRTabular(const State &state,
                                          const std::vector<double> &value,
                                          bool set_true) {
  AccumulateCFRTabular(
      CFRNetModel::TrainInputs{state.LegalActions(),
                               state.InformationStateTensor(), value},
      set_true);
}

void VPNetEvaluator::AccumulateCFRTabular(const CFRNetModel::TrainInputs &input,
                                          bool set_true) {
  for (auto &device : device_manager_.GetAll()) {
    if (value_or_policy) {
      device->AccumulateCFRTabularValue(input, set_true);
    } else {
      device->AccumulateCFRTabularPolicy(input, set_true);
    }
  }
}
...
std::vector<double> VPNetEvaluator::GetCFRTabular(const State &state,
                                                  bool get_true) {
  return GetCFRTabular(
      CFRNetModel::InferenceInputs{state.LegalActions(),
                                   state.InformationStateTensor()},
      get_true);
}

std::vector<double> VPNetEvaluator::GetCFRTabular(
    const CFRNetModel::InferenceInputs &input, bool get_true) {
  if (value_or_policy) {
    return device_manager_.Get(1, device_id_)
        ->GetCFRTabularValue(input, get_true);
  } else {
    return device_manager_.Get(1, device_id_)
        ->GetCFRTabularPolicy(input, get_true);
  }
}
