...
class TrainInput(collections.namedtuple(
        "TrainInput", "observation legals_mask target weight")):
    """Inputs for training the Model."""

    @staticmethod
    def stack(train_inputs):
        observation, legals_mask, target, weight = zip(*train_inputs)
        return TrainInput(
            np.array(observation, dtype=np.float32),
            np.array(legals_mask, dtype=np.int32),
            np.array(target),
            np.array(weight))
...
class CFRModel(object):
    ...
    """
    valid_model_types = ["normal", "baseline",
    ...
    def _define_graph(model_type, input_shape, output_size,
        ...
        elif model_type == "dualnet":
            targets = tf.placeholder(dtype=tf.float32, shape=[
                                     None, output_size * 2], name="targets")
            value_targets, policy_targets = tf.split(
                targets, [output_size, output_size], axis=-1)
        ...
        def _baseline_output_head(input_z, prefix=""):
            norm_z = tfkl.LayerNormalization()(tfkl.Dense(nn_width)(input_z))
            logit = tfkl.Dense(output_size, name=prefix +
                               "logit")(norm_z)
            baseline = tfkl.Dense(
                1, name=prefix + "baseline")(tfkl.Dense(nn_width)(input_z))
            return logit, baseline

        if model_type == "normal" or model_type == "normal_fc":
            logits = _output_head(z)
            logits = tf.where(masks, logits, tf.zeros_like(logits))
            ...
            logits = _output_head(z)
            logits = tf.where(masks, logits, -
                              1e32 * tf.ones_like(logits))
            policy = tf.nn.softmax(logits, name="policy")
            output = tf.identity(policy, name="output")
        elif model_type == "dualnet":
            value_logits = _output_head(z, "value_")
            policy_logits = _output_head(z, "policy_")
            value_logits = tf.where(
                masks, value_logits, tf.zeros_like(value_logits))
            value = tf.identity(value_logits, name="value")
            policy_logits = tf.where(
                masks, policy_logits, tf.zeros_like(policy_logits))
            policy = tf.identity(policy_logits, name="policy")
            output = tf.concat([value, policy], axis=-1, name="output")

            # losses
        if model_type == "normal" or model_type == "baseline" or model_type == "normal_fc":
            value_loss = 0.5 * tf.reduce_mean(
                weights * tf.squared_difference(value, targets))
        ...
        if model_type == "normal" or model_type == "normal_fc":
            ...
            value_loss = 0.5 * tf.reduce_mean(
                weights * tf.squared_difference(value, value_targets))
            policy_loss = 0.5 * tf.reduce_mean(
                weights * tf.squared_difference(policy, policy_targets))
            loss = tf.math.add(value_loss, policy_loss, name="loss")
        ...
        sim_train_op = optimizer.minimize(loss, name="sim_train")

    def inference(self, obs, masks):
        return self._session.run(self._output,
                                 feed_dict={self._input: obs, self._legals_mask: masks})
