...
DeepOSSBCFRSolver::RunIteration() {
...
std::pair<Trajectory, Trajectory> DeepOSSBCFRSolver::RunIteration(Player player,
                                                                  double alpha,
                                                                  int step) {
  return RunIteration(rng_, player, alpha, step);
}

std::pair<Trajectory, Trajectory> DeepOSSBCFRSolver::RunIteration(
...
double DeepOSSBCFRSolver::UpdateRegrets(
  ...
  if (step != 1 && (symmetry_ || cur_player != player)) {
    CFRNetModel::InferenceInputs inference_input{legal_actions,
                                                 information_tensor};
  ...
  if (cur_player == player) {
    ...
    //   std::vector<double> regret(legal_actions.size(), -value);
    //   regret[aidx] = (child_value - value);
    ...
    double delta = poker_state->MaxUtility() - poker_state->MinUtility();
    value_trajectory.states.push_back(ReplayNode{
        information_tensor, cur_player, legal_actions, aidx,
  ...
  if (cur_player == player && current_or_average[cur_player]) {
    ...
    if (use_policy_net) {
      policy_trajectory.states.push_back(
          ReplayNode{information_tensor, cur_player, legal_actions, aidx,
                     std::vector<double>{}, std::vector<Action>{}, -1, 1, 1.0,
                     player_reach, opponent_reach, sampling_reach});
    }
  }
  return value;
